...
import pl.clockworkjava.advanced.jpa.QueryResult;
...
//        System.out.println(resultList);
        TypedQuery<QueryResult> q = em.createQuery("SELECT new pl.clockworkjava.advanced.jpa.QueryResult(s.name, s.index.number) FROM Student s WHERE s.name IN ('John', 'Joanna')", QueryResult.class);

        TypedQuery<QueryResult> group = em.createQuery("SELECT new pl.clockworkjava.advanced.jpa.QueryResult(s.name, COUNT(s)) FROM Student s GROUP BY s.name", QueryResult.class);
